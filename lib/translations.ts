...
export function replaceNameForZh(text: string): string {
  ...
  let result = text
    .replace(/#かきょすぺーす/g, '#院院Space')
    .replace(/#心世紀スペース/g, '#心世纪Space')
    .replace(/#まいにちかきょボイス総集編/g, '#每日院院语音精选合集')
    .replace(/#まいにちかきょボイス/g, '#每日院院语音')
    .replace(/#おやすみかきょボイス/g, '#晚安院院语音')
    .replace(/#きょーのお話/g, '#院院的故事')
    .replace(/女子会トーク/g, '女生聚会杂谈')
    .replace(/初見歓迎歌枠/g, '欢迎初次见面歌会')
    .replace(/初見歓迎枠/g, '欢迎初次见面直播')
    .replace(/初見歓迎/g, '欢迎初次见面')
    .replace(/佳鏡院/g, '佳镜院')
...
export function replaceNameForEn(text: string): string {
  ...
  let result = text
    .replace(/#かきょすぺーす/g, '#KakyoSpace')
    .replace(/#心世紀スペース/g, '#ShinseikiSpace')
    .replace(/#まいにちかきょボイス総集編/g, '#DailyKakyoVoice Compilation')
    .replace(/#まいにちかきょボイス/g, '#DailyKakyoVoice')
    .replace(/#おやすみかきょボイス/g, '#GoodnightKakyoVoice')
    .replace(/#きょーのお話/g, '#KyoTalk')
    .replace(/女子会トーク/g, "Girls' Talk")
    .replace(/初見歓迎歌枠/g, 'Singing Stream (Welcome Newcomers)')
    .replace(/初見歓迎枠/g, 'Welcome Newcomers Stream')
    .replace(/初見歓迎/g, 'Welcome Newcomers')
    .replace(/佳鏡院/g, 'Kakyo')
    ...
    .replace(/ボイス/g, 'Voice')
    .replace(/カバー/g, 'Cover')
    .replace(/コラボ/g, 'Collab')
    .replace(/お話/g, 'Talk')
    .replace(/雑談/g, 'Chat Stream')
    ...
    .replace(/万人達成/g, '0K Subscribers')
    .replace(/新衣装披露/g, 'New Outfit Stream')
    .replace(/3Dお披露目/g, '3D Reveal Stream')
    .replace(/すぺーす/g, 'Space')
    .replace(/総集編/g, 'Compilation')
    .replace(/出演/g, 'Appearance')
    .replace(/開催/g, 'Event')
    .replace(/リリース/g, 'Release')
    .replace(/さん/g, '')
